...
func main() {
	...
	*/
	...
	resultChannel := make(chan []map[byte][]float64, numGoRoutines+1)
...
func ProcessByStrips(resultChan chan []map[byte][]float64, chunkChannel chan Chunk, threadID, numberOfThreads int, wg *sync.WaitGroup) {
	var rowCount int
	...
	Q1HashAgg := func(rowData []LineItemRow) {
		for _, row := range rowData {
			if row.l_shipdate <= datePredicate {
				...
				if _ ,ok := fr[0][res1]; !ok {
					fr[0][res1] = make([]float64, 10)
				}
				if _, ok := fr[1][res2]; !ok {
...
func AccumulateResultSet(partialResult []map[byte][]float64, fr []map[byte][]float64) []map[byte][]float64 {
...
func FinalizeResultSet(partialResult []map[byte][]float64) []map[byte][]float64 {
